...
template <class Grain> class GrainIterator {
  public:
    ...
    explicit GrainIterator(Grain *grainPtr)
        : _grainPtr(grainPtr), _position(0){};

    GrainIterator(Grain *grainPtr, size_t position)
        : _grainPtr(grainPtr), _position(position){};
    ...
    value_type operator*() const { return (*_grainPtr)[_position]; };

    value_type operator->() const { return (*_grainPtr)[_position]; };

    friend bool operator==(const GrainIterator &,
                           const GrainIterator &) = default;
    friend bool operator!=(const GrainIterator &,
                           const GrainIterator &) = default;
...
template <class Window> class Grain {
  public:
    explicit Grain(std::span<float> samples)
        : _samples(samples), _window(samples.size()) {}
