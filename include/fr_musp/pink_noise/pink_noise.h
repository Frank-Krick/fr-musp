...
double calculate_noise_sample_sum(const std::array<double, _number_of_white_noise_sources> &noiseSamples) {
    return std::accumulate(noiseSamples.begin(), noiseSamples.end(), 0) / (double) _number_of_white_noise_sources;
}
...
double update_noise_samples_and_calculate_new_sum(
    ...
    return currentSum + (firstRandom + secondRandom - oldValues.first - oldValues.second) / (double) _number_of_white_noise_sources;
}
...
class PinkNoise {
public:
    PinkNoise(): _currentCounter(0), _maximumCounter(maximum_counter_value<_number_of_white_noise_sources>()) {
    ...
    double operator()() {
        ++_currentCounter;
        if (_currentCounter > _maximumCounter) _currentCounter = 1;
